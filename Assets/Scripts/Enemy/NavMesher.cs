...
namespace MyGame.Enemy
{
    public class NavMesher
    {
        ...
        {
            var currentPoint = _navMeshPath.corners[_currentPathPointIndex];            // получаем текущую целевую точку пути
            var distance = (_agentTransform.position - currentPoint).magnitude;         // расстояние между агентом (врагом) и текущей точкой пути

            if (distance < DistanceEps)                 // если мы достаточно близко к текущей целевой точке, то переходим на следующую точку (след. индекс)
                _currentPathPointIndex++;

            if (_currentPathPointIndex >= _navMeshPath.corners.Length)          // проверяем есть ли еще точки на пути (дошли ли мы до цели?)
                IsPathCalculated = false;                                       // если нет => мы дошли до конца пути
            else 
                currentPoint = _navMeshPath.corners[_currentPathPointIndex];    // если да => берем эту следующую целевую точку
